...
#Using eqs on page 8 of Barfield et al biorxiv
mr <- function(dat, pval_thresh=1e-3, p_val_column="p_value"){
    dat <- dat %>%
    ...
    if(nrow(dat) < 2){
       ...
       return(R)
        ...
        #gamma_hat <- with(dat, sum(beta_hat_1*beta_hat_2/(seb1^2))/ sum((beta_hat_1^2)/(seb1^2)))
        #sigma_sq_mr <- with(dat, sum( ((beta_hat_2 - (beta_hat_1 *gamma_hat))^2)/(seb2^2))/(nrow(dat)-1))
        #var_gamma_hat <- with(dat, sigma_sq_mr/sum((beta_hat_1^2)/(seb2^2)))
        #z_mr <- gamma_hat/sqrt(var_gamma_hat)
        #return(z_mr)
    #Equivalent to
...
mr_egger <- function(dat, pval_thresh=1e-3, p_val_column="p_value"){
    dat <- dat %>%
            filter(get(p_val_column) < pval_thresh)
    if(nrow(dat) < 3){
       R <- list("z"=NA, "p" = NA)
...
mr_mrpresso <- function(dat, pval_thresh = 1e-3, p_val_column = "p_value"){
    dat <- dat %>%
            filter(get(p_val_column) < pval_thresh)
    ...
    R <- list(t = t_mrp, p = p_mrp)
    return(R)

}

#Gusev et al 2016
twas <- function(dat, pval_thresh=1e-3, p_val_column="p_value"){
    dat <- dat %>%
            filter(get(p_val_column) < pval_thresh)
    if(nrow(dat) < 2){
       R <- list("z"=NA, "p" = NA)
...
weighted_median <- function(dat, pval_thresh=1e-3, p_val_column="p_value"){
    dat <- dat %>%
            filter(get(p_val_column) < pval_thresh)
    if(nrow(dat)< 3){ 
        ret <- list(z = NA,  p = NA) 
...
pweighted_median <- function(dat, pval_thresh=1e-3, p_val_column="p_value"){
    dat <- dat %>%
            filter(get(p_val_column) < pval_thresh)
    if(nrow(dat)< 3){ 
        ret <- list(z = NA,  p = NA) 
